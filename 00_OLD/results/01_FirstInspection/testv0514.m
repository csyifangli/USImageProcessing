...
% and #2
trainus(X_1,ys_1,X_1,ys_1,OO1,GR1,debg,1,grid_debug,radius_debug);
%trainus(X_1,ys_1,X_2,ys_2,OO1,GR2)
% Training with the on-off force #1, testing on the graded force #1
% and #2
trainus(X_1,yf_1,X_1,yf_1,OO1,GR1,debg,2,grid_debug,radius_debug);
...
function trainus(Xt,yt,Xp,yp,tset,pset,debg, forcestim,grid_debug,radius_debug)
...
for k=1:5 % finger to be analyzed      
        ...
        nMSE(end+1) = mean( (y_test-y_hat').^2 );
        if debg
            load nmseanalysis0514.mat
            if forcestim == 1   %stimulus
                data{k}.stimulusv2(find(grid_debug==grid_length),find(radius_debug==radius_circle))=nMSE(end);
            elseif forcestim == 2   %force
                data{k}.forcev2(find(grid_debug==grid_length),find(radius_debug==radius_circle))=nMSE(end);
            end
