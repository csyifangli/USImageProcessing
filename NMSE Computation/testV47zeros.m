...
maximumsCell = cell(1,1);
maximumsCell2 = cell(1,1);
features1 = cell(1,1);
features2 = cell(1,1);

features1{1} = load('E:\data\claudio\1\features\Grid543Circle50.txt');

features2{1} = load('E:\data\claudio\2\features\Grid543Circle50.txt');
...
% and #2
    ...
    nMSE = trainus(X_1,ys_1,X_1,ys_1,OO1,GR1,maximumsCell{i,j});
    nMSE2 = trainus(X_2,ys_2,X_2,ys_2,OO2,GR2,maximumsCell2{i,j});
...
for k=1:5 % finger to be analyzed      
        ...
        nMSE{k}.error = sqrt(( (y_test-y_hat').^2 ))/(max(yt(:,k))-min(yt(:,k)));
